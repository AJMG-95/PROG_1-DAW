''' Lavadora (4Pts)
    ...
    La lavadora va pasando por todos los programas de lavado desde el
      ...
      el programa 1, pasaría por los programas 1 -> 2 -> 3 -> 4 -> 0.
      Si empieza por el 2, pasaría por 2 -> 3 -> 4 -> 0.
...
class Deposito:
    ...
    def llenar(self, litros):
        if (self.__cantidad + litros) > self.__capacidad:
            self.__cantidad = self.__capacidad
        else:
            self.__cantidad += litros
        return self

    def quitar(self, litros):
        if self.__cantidad <= litros:
            self.__cantidad = 0
        else:
            self.__cantidad -= litros
        return self
...
class Lavadora:
    def __init__(self):
        ...
        self.__programa = 0
        self.__puerta = True
        self.__detergente = Deposito(3)
        self.__suavizante = Deposito(5)

    def detergente(self):
        return self.__detergente

    def suavizante(self):
        return self.__suavizante

    def encender(self):
        if self.__detergente.restante() >= 0.8 and self.__suavizante.restante() >= 0.5 and self.__puerta == False:
    ...
    def abrir(self):
        if self.__programa == 0:
            self.__puerta = True
            return self

    def cerrar(self):
        if self.__puerta == True:
            self.__puerta = False
        return self
    ...
    def continuar_programa(self):
        if self.__programa != 0 and self.__encendida == True:
            if self.__programa == 4:
                self.__programa = 0
            else:
                self.__programa += 1
                self.__detergente.quitar(0.4)
                self.__suavizante.quitar(0.2)
        return self
